...
Design notes:
    ...
    - Atomic .env rewrite: tempfile -> flush -> fsync -> close -> rename.
      Explicit fsync closes the crash window between rename and default sync.
...
def _setup_logger() -> logging.Logger:
    """Configure structured logger (JSON-ish per line) to file + stdout."""
    ...
    for name in COOKIE_TO_ENV:
        ...
        if prefix == b"v10":
            raise RuntimeError(
                ...
                "See cookie_bridge/README.md § future-encryption."
            )
        if prefix == b"v11":
            raise RuntimeError(
                f"cookie {name!r} is v11-encrypted — snap gained libsecret access via password-manager-service. "
                "Bridge needs libsecret key lookup. See README § future-encryption."
            )
...
def _atomic_write_env(path: Path, content: str) -> None:
    ...
    and the kernel's eventual writeback.
    """
