...
4. Validate lengths + printable-ASCII.
...
falls through to **plaintext storage in the `value` column** — `encrypted_value`
is 0 bytes for every TV cookie. The bridge reads `value` directly.
...
| `cookie_bridge/state.json` | `{"consecutive_failures", "last_alert_ts"}` for alert throttling |
